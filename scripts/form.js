function formatUsers(users) {
  // Create an object to keep track of IDs that have already been used
  var usedIds = {};
  let index =0;
  var formatted=[];
 for(let i = 0; i < users.length; i++)
 {
   ...
   formatted[i]=userobject;
 };
 // Combine the formatted array with additional_users array and remove duplicates
 const allUsers = formatted.concat(additionalUsers.filter(addUser => {
   return !formatted.some(user => user.full_name === addUser.full_name);
 }));
  const courses = [
    "Mathematics",
    "Physics",
 ...
 {
  // Loop through the users array and add the new fields
  let id;
  id = index+1;
  index++;
  var favorite;
  if(Math.random()<0.5)
    ...
    favorite = true;
  // Generate a random course for the user
  const course = courses[Math.floor(Math.random() * courses.length)];
  var bg_color = "#"+Math.floor(Math.random()*16777215).toString(16);
  // Create the formatted user object with the new fields
  allUsers[i]= {
   id,
   favorite,
   course,
 ...
 //console.log("Validity of user with id "+(i+1)+": "+isValidObject(allUsers[i]));
 };
 //console.log(allUsers);
 return allUsers;
};
function isValidObject(user){
  let pattern=/^[A-ZÄÖÜß][a-zäöüß]+(\s[A-ZÄÖÜß][a-zäöüß]+)*$/;
  var allresults;
  let result = pattern.test(user.full_name);
  allresults=result+" ";
  pattern=/^[a-zäöüß]*$/;
  result=pattern.test(user.gender);
  allresults+=result+" ";
  pattern =/^[A-ZÄÖÜß][a-zäöüß]*(-[A-ZÄÖÜß][a-zäöüß]+)*( [A-ZÄÖÜß]*[a-zäöüß]+)*( [A-ZÄÖÜß]*[a-zäöüß]+)?$/;
  result = pattern.test(user.note);
  allresults+=result+" ";
  result = pattern.test(user.state);
  allresults+=result+" ";
  result = pattern.test(user.city);
  allresults+=result+" ";
  result = pattern.test(user.country);
  allresults+=result+" ";
  pattern=/^[0-9]+$/;
  result = pattern.test(user.age);
  allresults+=result+" ";
  pattern=/^\S+@\S+\.\S+$/;
  result = pattern.test(user.email);
  allresults+=result+" ";
  return allresults;
};
function filterObjects(arr, country, minAge, maxAge, gender, favorite) {
  return arr.filter(obj => 
    (country ? obj.country === country : true) &&
    (minAge ? obj.age >= minAge : true) &&
    (maxAge ? obj.age <= maxAge : true) &&
    (gender ? obj.gender === gender : true) &&
    (favorite ? obj.favorite === favorite : true)
  );
};
function sortArrayOfObjects(arr, sortParam, sortOrder = 'asc') {
  const sortOrderMultiplier = sortOrder === 'desc' ? -1 : 1;
  function compareObjects(a, b) {
    if (typeof a[sortParam] === 'string' && typeof b[sortParam] === 'string') {
      return sortOrderMultiplier * a[sortParam].localeCompare(b[sortParam]);
    } else {
      return sortOrderMultiplier * (a[sortParam] - b[sortParam]);
    }
  }
  return arr.sort(compareObjects);
};
function findObjectByParameter(array, param, paramValue) {
  let foundusers=[];
  let j=0;
  for (let i = 0; i < array.length; i++) {
    const obj = array[i];
    if (obj[param] === paramValue) {
      foundusers[j]=obj;
      j++;
    }
  }
...
function percentageOfMatching(array, matchedarray) {
  return matchedarray.length / array.length * 100;
}
...
const arrusers=formatUsers(randomUserMock);
//console.log(arrusers);
//console.log(filterObjects(arrusers,"United States", 18, null, "male", true));
//console.log(sortArrayOfObjects(arrusers, "country", 'asc'));
//console.log(findObjectByParameter(arrusers, "age", 28));
//console.log(percentageOfMatching(arrusers, filterObjects(arrusers, null, 30, 50, 'female', null)));